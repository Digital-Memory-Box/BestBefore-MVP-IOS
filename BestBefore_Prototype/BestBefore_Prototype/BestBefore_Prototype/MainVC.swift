...
// Simple local store for rooms (persisted in UserDefaults)
...
//
//enum RoomsStore {
//    private static let key = "bb.rooms.meta"
//
//    static func all() -> [RoomMeta] {
//        guard let data = UserDefaults.standard.data(forKey: key) else { return [] }
//        return (try? JSONDecoder().decode([RoomMeta].self, from: data)) ?? []
//    }
//
//    static func save(_ rooms: [RoomMeta]) {
//        let data = try? JSONEncoder().encode(rooms)
//        UserDefaults.standard.set(data, forKey: key)
//    }
//
//    static func add(_ room: RoomMeta) {
//        var rooms = all()
//        rooms.insert(room, at: 0)
//        save(rooms)
//    }
//    static func update(_ room: RoomMeta) {
//        var rooms = all()
...
class LoginPaddedTextField: UITextField {
...
class MainViewController: UIViewController, UITextFieldDelegate {
    ...
    private let emailTextField: LoginPaddedTextField = {
        let textField = LoginPaddedTextField()
    ...
    private let passwordTextField: LoginPaddedTextField = {
        let textField = LoginPaddedTextField()
    ...
    @objc private func createAccountTapped() {
        ...
        let alert = UIAlertController(
            ...
            message: "Account creation would be implemented here.",
            preferredStyle: .alert
        )
        alert.addAction(UIAlertAction(title: "OK", style: .default, handler: nil))
        present(alert, animated: true, completion: nil)
    ...
    private func attemptLogin() {
        ...
        } else if !isValidEmail(email) && !isValidNickname(email) {
            print("Invalid email or nickname format.")
            shakeView(emailTextField)
        } else {
            // After login, let the user create a new room or pick an existing one
    ...
    private func presentRoomsChoice() {
        Task { [weak self] in
            guard let self = self else { return }
            do {
                let rooms = try await Database.shared.getAllRooms()
                await MainActor.run { self.presentRoomsChoiceSheet(with: rooms) }
            } catch {
                await MainActor.run {
    ...
    private func presentRoomsChoiceSheet(with rooms: [RoomObject]) {
        ...
        if !rooms.isEmpty {
            sheet.addAction(UIAlertAction(title: "Select existing…", style: .default, handler: { _ in
                let picker = RoomsPickerController(rooms: rooms) { selectedRoom in
                    self.presentRoomScene(for: selectedRoom)
                }
                if #available(iOS 15.0, *) { picker.modalPresentationStyle = .pageSheet }
    ...
    private func promptForRoomName() {
        ...
        alert.addAction(UIAlertAction(title: "Create", style: .default, handler: { [weak self] _ in
            ...
            let finalName = (name?.isEmpty == false) ? name! : "Untitled Room"
            Task { [weak self] in
                guard let self = self else { return }
                do {
                    try await Database.shared.addRoom(name: finalName, ownerEmail: self.lastLoggedInEmail)
                    let rooms = try await Database.shared.getAllRooms()
                    await MainActor.run {
                        if let latest = rooms.first {
                            self.presentRoomScene(for: latest)
                        } else {
                            self.presentRoomsChoiceSheet(with: rooms)
                        }
                    }
                } catch {
                    await MainActor.run {
                        let errAlert = UIAlertController(title: "Room Error",
    ...
    private final class RoomsPickerController: UITableViewController {
        ...
        override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
            ...
            if #available(iOS 14.0, *) {
                ...
                let df = DateFormatter()
                df.dateStyle = .medium; df.timeStyle = .short
                cell.detailTextLabel?.text = df.string(from: r.createdAt)
            }
